...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/09/03 12:43:08 by dadler            #+#    #+#             */
/*   Updated: 2019/09/10 11:42:10 by dadler           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	print_params2(t_vm *v, t_process p, t_param *param_tab, int i)
{
	...
	if (param_tab[i].indirect)
	{
			...
			|| p.opc == 11 || p.opc == 14)
		{
			...
			ft_putnbr(reverser_16(read_ind(v, tmp + p.pc, IND_SIZE)));
		else
			ft_putnbr(tmp);
		return (1);
	}
...
void		print_op_exec(t_vm *v, t_plist *list, t_param *param_tab)
{
	...
	if (list->proc.opc == 12)
		ft_printf(" (%d)",
			(list->proc.pc + (reverser_16(param_tab[0].val) % IDX_MOD)
				% MEM_SIZE));
	if (list->proc.opc == 15)
		ft_printf(" (%d)",
